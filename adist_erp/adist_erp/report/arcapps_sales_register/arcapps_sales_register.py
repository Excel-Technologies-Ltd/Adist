...
def get_invoices(filters, additional_query_columns=None):
    # base fields
    fields = [
        frappe._dict(label="doctype", fieldname="doctype", fieldtype="Data", options=None),
        "name",
        "posting_date",
        "debit_to",
        "project",
    ...
    # add constant doctype value
    # frappe.get_list doesn’t support ConstantColumn, so we set manually after query
    fields = [f for f in fields if not isinstance(f, dict)]  # ensure clean list
    if additional_query_columns:
        fields.extend(additional_query_columns)

    # build filters dict
    flt_dict = {"docstatus": 1}
    if filters.get("customer"):
        flt_dict["customer"] = filters.get("customer")

    if filters.get("owner"):
        flt_dict["owner"] = filters.get("owner")

    # mode_of_payment filtering needs join in QB; with get_list we can’t join directly
    # so skip it here unless we do post-filter or custom SQL
    ...
    # manually set doctype column value
    for inv in invoices:
