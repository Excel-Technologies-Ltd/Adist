...
def get_data_with_opening_closing(filters, account_details, accounting_dimensions, gl_entries):
    ...
    # Initialize a map to store GL entries grouped by account
    gle_map = initialize_gle_map(gl_entries, filters)

    totals, entries = get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map)

    # Add opening balance for all accounts, including child accounts with no transactions
    data.append({
        "account": _("Total Opening Balance"),  # Account name is now "Total Opening"
    ...
    for acc, acc_dict in gle_map.items():
        # For accounts with no transactions, display the opening balance without consolidation
        if not acc_dict.entries:
            # Add an empty row before the opening balance for accounts with no transactions
            data.append({
                "account": "",  # Empty row for account name
                "label": "",  # Empty row for label
                "debit": 0,  # Empty row for debit (use 0 instead of "")
                "credit": 0,  # Empty row for credit (use 0 instead of "")
                "balance": 0,  # Empty row for balance (use 0 instead of "")
            })

            # Show the opening balance for accounts with no transactions
            data.append({
                "account": acc,  # Account name
                "label": _("Opening Balance"),  # Label for clarity
                "debit": acc_dict.totals.opening.debit,
                "credit": acc_dict.totals.opening.credit,
                "balance": acc_dict.totals.opening.debit - acc_dict.totals.opening.credit,  # Calculate balance
            })

            # Add an empty row after the opening balance
            data.append({
                "account": "",  # Empty row for account name
                "label": "",  # Empty row for label
                "debit": 0,  # Empty row for debit (use 0 instead of "")
            ...
            # Show the opening balance for accounts with transactions
            data.append({
                "account": acc,  # Account name
                "label": _("Opening Balance"),  # Label for clarity
                "debit": acc_dict.totals.opening.debit,
            ...
            # Show totals and closing for accounts with transactions
            data.append({
                "account": acc,  # Account name
                "label": _("Total"),  # Label for Total
                "debit": acc_dict.totals.total.debit,
                "credit": acc_dict.totals.total.credit,
                "balance": acc_dict.totals.total.debit - acc_dict.totals.total.credit,  # Total balance
            })
            data.append({
                "account": acc,  # Account name
                "label": _("Closing (Opening + Total)"),  # Label for Closing
                "debit": acc_dict.totals.closing.debit,
                "credit": acc_dict.totals.closing.credit,
                "balance": acc_dict.totals.closing.debit - acc_dict.totals.closing.credit,  # Closing balance
            })

    # Add totals and closing for consolidated view
    data.append({
        "label": _("Consolidated Total"),
        "debit": totals.total.debit,
        "credit": totals.total.credit,
        "balance": totals.total.debit - totals.total.credit,
    })
    data.append({
        "label": _("Consolidated Closing"),
