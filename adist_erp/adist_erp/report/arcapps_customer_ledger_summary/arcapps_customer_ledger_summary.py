...
class PartyLedgerSummaryReport:
	...
	def get_additional_columns(self):
		...
		if self.filters.party_type == "Customer":
			...
			# Use frappe.get_all instead of direct QB to respect user permissions
			customers = frappe.db.get_list(
				"Customer",
				filters={"disabled": 0},
				fields=["name", "territory", "customer_group", "default_sales_partner"],
			)

			for customer in customers:
				self.territories[customer.name] = customer.territory
				self.customer_group[customer.name] = customer.customer_group
		else:
			...
			for supplier in suppliers:
				self.supplier_group[supplier.name] = supplier.supplier_group
	...
	def get_data(self):
		...
		# Get allowed parties based on user permissions
		allowed_parties = self.get_allowed_parties()

		self.party_data = frappe._dict({})
		for gle in self.gl_entries:
			# Skip if party is not in allowed parties (user permission check)
			if allowed_parties and gle.party not in allowed_parties:
				continue
	...
	def get_allowed_parties(self):
		"""
		Get parties that the current user has permission to view
		Returns None if user has access to all parties, otherwise returns a set of allowed parties
		"""
		try:
			# Use frappe.get_all to get parties respecting user permissions
			if self.filters.party_type == "Customer":
				parties = frappe.db.get_list("Customer", pluck="name")
			else:
				parties = frappe.db.get_list("Supplier", pluck="name")
			
			return set(parties) if parties else set()
		except frappe.PermissionError:
			# If user has no permission, return empty set
			return set()
