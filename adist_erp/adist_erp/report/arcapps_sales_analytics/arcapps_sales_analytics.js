	...
	filters: [
		{
			...
			reqd: 1,
			on_change: function() {
				let tree_type = frappe.query_report.get_filter_value('tree_type');
				let doc_type = frappe.query_report.get_filter_value('doc_type');
				
				// Disable Item and Item Group when Payment Entry is selected
				if (doc_type === 'Payment Entry' && (tree_type === 'Item' || tree_type === 'Item Group' || tree_type === 'Order Type')) {
		...
		},
		{
			...
			label: __("based_on"),
			fieldtype: "Select",
			options: ["Sales Order", "Delivery Note", "Sales Invoice", "Payment Entry"],
			...
			on_change: function() {
				...
				// Reset tree type if incompatible with Payment Entry
				if (doc_type === 'Payment Entry' && (tree_type === 'Item' || tree_type === 'Item Group' || tree_type === 'Order Type')) {
					frappe.msgprint(__("Item, Item Group, and Order Type analytics are not available for Payment Entry. Defaulting to Customer tree type."));
					frappe.query_report.set_filter_value('tree_type', 'Customer');
				}
				
				// Adjust value_quantity filter options for Payment Entry
				if (doc_type === 'Payment Entry') {
					let value_quantity_filter = frappe.query_report.get_filter('value_quantity');
					value_quantity_filter.df.options = [
						{ value: "Value", label: __("Value") }
					];
					value_quantity_filter.refresh();
					frappe.query_report.set_filter_value('value_quantity', 'Value');
				} else {
					let value_quantity_filter = frappe.query_report.get_filter('value_quantity');
					value_quantity_filter.df.options = [
						{ value: "Value", label: __("Value") },
